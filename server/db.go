...
func (s *DB) SaveOrganization(project bitbucketv1.Project) error {
	...
	_, err := s.tx.Exec(statement,
		...
		false,                      // two_factor_requirement_enabled boolean,
...
func (s *DB) SaveUser(user bitbucketv1.User) error {
	statement := fmt.Sprintf(
...
func (s *DB) SaveRepository(owner bitbucketv1.Project, repository bitbucketv1.Repository) error {
	statement := fmt.Sprintf(
	...
	_, err := s.tx.Exec(statement,
		...
		owner.ID,                       // owner_id bigint NOT NULL,
		owner.Key,                      // owner_login text NOT NULL,
...
func (s *DB) SavePullRequest(repositoryOwner, repositoryName string, pr PullRequest) error {
	statement := fmt.Sprintf(
...
func (s *DB) SavePullRequestComment(repositoryOwner, repositoryName string, pullRequestNumber int, comment Comment) error {
...
func (s *DB) SaveIssueComment(repositoryOwner, repositoryName string, issueNumber int, comment Comment) error {
	...
	_, err := s.tx.Exec(statement,
		...
		repositoryName,                           // repository_name text NOT NULL,
		repositoryOwner,                          // repository_owner text NOT NULL,
		time.Unix(int64(comment.UpdatedDate), 0), // updated_at timestamptz,
